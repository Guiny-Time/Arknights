...
public class AStarMgr : BaseManager<AStarMgr>
{
    ...
    /// <param name="w"></param>
    /// <param name="h"></param>
    public void InitMapInfo(string dataPath,int w ,int h) 
    {
        //记录宽高 范围
        this.mapW = w;
        this.mapH = h;
        ...
        if(File.Exists(Application.dataPath+dataPath)){
            ...
            for ( int i = 0; i < w; i++ )
            {
                for( int j = 0; j < h; j++ )
                {
                    string Type = sr.ReadLine();//暂时储存格子通行信息的变量
                    E_Node_Type nodeType = (E_Node_Type)System.Enum.Parse(typeof(E_Node_Type), Type);
